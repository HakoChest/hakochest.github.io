...
`vielpork`提供了一个内置的TUI报告器`TuiReporter`，用于在终端中显示下载任务的进度。`TuiReporter`实现了`ProgressReporter`和`ResultReporter`两个trait，用于向外部报告下载任务的进度和结果。

`TuiReporter`使用`indicatif`库来显示下载任务的进度。在下载任务开始时，`TuiReporter`会创建一个新的进度条，并在下载任务结束时更新进度条的状态。`TuiReporter`还会在下载任务结束时向外部报告下载任务的结果。
